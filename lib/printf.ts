...
function sprintf(...vnarg: any[]) {
    ...
    return format.replace(sprintf.regex, function(substring, valueIndex, flags, minWidth, _, precision, type) {
        ...
        let value: string | number = valueIndex ? a[valueIndex.slice(0, -1)] : a[i++];
        let prefix, base;
        ...
        switch (type) {
            ...
            case 'P': {
                ...
                if ('p' != type.toLowerCase()) {
                    ...
                    let sf: string | number = String(value).replace(/[eE].*|[^\d]/g, '');
        ...
        let justified = justify(value, prefix, leftJustify, minWidth, zeroPad);
...
/**
 * Trivial printf implementation, probably only useful during page-load.
 * Note: you may as well use "document.write(sprintf(....))" directly
 */
function printf() {
    // delegate the work to sprintf in an IE5 friendly manner:
    let i = 0, a = arguments, args = Array(arguments.length);
